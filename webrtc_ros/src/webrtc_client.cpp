...
namespace webrtc_ros
{
  ...
  public:
    ...
    void handle_message(MessageHandler::Type type, const std::string &raw)
    {
      ...
      if (_this)
        _this->signaling_thread_->BlockingCall([t = _this.get(), type, raw]
                                               { t->handle_message(type, raw); });
    }
...
#define FIND_PROPERTY_OR_CONTINUE(key, name)                               \
  ...
  std::string name = action.properties.at(key)
          ...
          if (action.type == ConfigureAction::kRemoveStreamActionName)
          {
            ...
            if (video_type == "ros_image")
            {
              ...
              rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track(
                  peer_connection_factory_->CreateVideoTrack(
                      track_id,
                      capturer.get()));
              // Unified Plan: PeerConnectionに直接AddTrack
              std::vector<std::string> stream_ids = {stream_id};
              auto result = peer_connection_->AddTrack(video_track, stream_ids);
              if (!result.ok())
              {
                RCLCPP_WARN_STREAM(nh_->get_logger(), "AddTrack failed: " << result.error().message());
                continue;
              }
              capturer->Start();
            }
