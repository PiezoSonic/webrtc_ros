...
namespace webrtc_ros
{
      ...
      : nh_(nh), itf_(nh, std::make_shared<image_transport::ImageTransport>(nh))
  {
    ...
    nh_->declare_parameter<std::string>("image_transport", "raw");
    nh_->get_parameter("image_transport", image_transport_);
    int port;
    nh_->declare_parameter<int>("port", 8080);
    port = nh_->get_parameter("port").as_int();
